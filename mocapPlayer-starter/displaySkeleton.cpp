...
float DisplaySkeleton::boneColors[NUMBER_BONE_COLORS][3] = {
    {0.7f, 1.0f, 0.7f}, // LIGHTGREEN
    {1.0f, 0.7f, 0.7f}, // LIGHTRED
    {0.7f, 0.7f, 1.0f} // LIGHTBLUE
};
...
void DisplaySkeleton::SetDisplayList(int skeletonID, Bone *bone, GLuint *pBoneList)
{
  ...
  *pBoneList = glGenLists(numbones);
  ...
  colorIndex = numSkeletons % NUMBER_BONE_COLORS;
  float boneShininess = 120.0f;
  float boneAmbient[4] = {ambientFskeleton * boneColors[colorIndex][0], ambientFskeleton * boneColors[colorIndex][1], ambientFskeleton * boneColors[colorIndex][2], 1.0};
  float boneDiffuse[4] = {diffuseFskeleton * boneColors[colorIndex][0], diffuseFskeleton * boneColors[colorIndex][1], diffuseFskeleton * boneColors[colorIndex][2], 1.0};
  float boneSpecular[4] = {specularFskeleton * boneColors[colorIndex][0], specularFskeleton * boneColors[colorIndex][1], specularFskeleton * boneColors[colorIndex][2], 1.0};
  ...
  for(int j=0;j<numbones;j++)
  {
    ...
    if (j == 16) {
        ...
        gluCylinder(qobj, boneRadius*1.8, boneRadius*1.8, bone[j].length, 20, 20);
        // Two disks to close the cylinder at the bottom and the top
        gluDisk(qobj, 0.0, boneRadius*1.8, 20, 20);
        glPushMatrix();
