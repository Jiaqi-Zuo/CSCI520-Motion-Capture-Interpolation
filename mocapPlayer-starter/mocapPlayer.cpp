...
int clickedCnt = 0;
int colorChangedCnt = 0;
// defalut color of ground
double groundR = 0.81;
double groundG = 0.81;
double groundB = 0.55;
...
std::vector<std::vector<double>> groundColors = {
    {0.55f,0.56f,0.81f},
    {0.68f,0.81f,0.55f},
    {0.61f,0.55f,0.55f}
};
...
int texClickCnt = 0;
int texChangedCnt = 0;
bool groundTexClicked = false;
std::vector<std::string> groundTexturePics = {
    "./texture/01.jpg","./texture/02.jpg","./texture/03.jpg","./texture/04.jpg"
};
...
std::vector<std::vector<float>> positionGround;
std::vector<std::vector<float>> texCoordGround;
bool groundSaved = false;

std::vector<std::string> faces = 
{
    "./skybox/right.jpg", "./skybox/left.jpg", "./skybox/top.jpg", "./skybox/bottom.jpg", 
    "./skybox/front.jpg", "./skybox/back.jpg"
};

unsigned int loadCubemap(std::string currPic)
{
    GLuint textureID;
    int width, height, nrChannels;
    //const char* imagePath = (currTexture).c_str();
    unsigned char* data = stbi_load((currPic).c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
...
void initTextureVAO_VBO() {
    GLuint textureID;
    int width, height, nrChannels;
    //const char* imagePath = (currTexture).c_str();
    unsigned char* data = stbi_load((currTexture).c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
        //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        printf("Loaded image with a width of %dpx, a height of %dpx and %d channels\n", width, height, nrChannels);
        //gluBuild2DMipmaps(GL_TEXTURE_2D, 3, width, height, GL_RGB, GL_UNSIGNED_BYTE, data); // texture mipmaps
    ...
    for (int i = 0; i < positionGround.size(); i++) {
        glTexCoord2f(texCoordGround[i][0], texCoordGround[i][1]);
        glVertex3f(positionGround[i][0], positionGround[i][1], positionGround[i][2]);
    }
    glEnd();
    glDisable(GL_TEXTURE_2D); 
    stbi_image_free(data);
...
void RenderGroundPlane(double groundPlaneSize, double groundPlaneHeight, double rPlane, double gPlane, double bPlane, double ambientFskeleton, double diffuseFskeleton, double specularFskeleton, double shininess)
{
  ...
  for(int i=0; i<planeResolution; i++)
    for(int j=0; j<planeResolution; j++)
    {
      ...
      if (!groundSaved) {          
          positionGround.push_back({ (float)(-groundPlaneSize / 2 + i * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + j * planeIncrement) });
          positionGround.push_back({ (float)(-groundPlaneSize / 2 + i * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + (j + 1) * planeIncrement) });
          positionGround.push_back({ (float)(-groundPlaneSize / 2 + (i + 1) * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + (j + 1) * planeIncrement) });

          positionGround.push_back({ (float)(-groundPlaneSize / 2 + (i + 1) * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + (j + 1) * planeIncrement) });
          positionGround.push_back({ (float)(-groundPlaneSize / 2 + (i + 1) * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + j * planeIncrement) });
          positionGround.push_back({ (float)(-groundPlaneSize / 2 + i * planeIncrement), (float)groundPlaneHeight, (float)(-groundPlaneSize / 2 + j * planeIncrement) });
          // triangle (0,1,2)
          texCoordGround.push_back({ 0.0f, 0.0f });
          texCoordGround.push_back({ 0.0f, 1.0f });
          texCoordGround.push_back({ 1.0f, 1.0f });
          // triangle (2,3,0)
          texCoordGround.push_back({ 1.0f, 1.0f });
          texCoordGround.push_back({ 1.0f, 0.0f });
          texCoordGround.push_back({ 0.0f, 0.0f });
      }
...
void Redisplay() 
{
  ...
  glClearColor(0.53f, 0.81f, 0.92f, 0);
  ...
  { 
    if (useFog == ON)
    {
      ...
      GLfloat fogColor[4] = {0.53f,0.81f,0.92f,0.2};
...
void saveScreenshot(int windowWidth, int windowHeight, const char * filename);
...
void saveScreenToFile_callback(Fl_Button *button, void *)
{
  if (button == screenShot_button)
  {
    ...
    saveScreenshot(640, 480, ("./screenshots/" + std::string(saveScreenToFileOnceFilename)).c_str());
...
void idle(void*)
{
  ...
  if(playButton == ON) 
  {
    if (saveScreenToFile == SAVE_CONTINUOUS)
    {
      ...
      saveScreenshot(640, 480, ("./screenshots/" + std::string(saveScreenToFileContinuousFilename)).c_str());
...
void groundTex_callback(Fl_Button* button, void*) {
    if (groundPlane == ON) {
        groundTexClicked = true;
        currTexture = groundTexturePics[texChangedCnt];
        texChangedCnt += 1;
        if (texChangedCnt >= groundTexturePics.size()) texChangedCnt = 0;
    }
    ...
    RenderGroundPlane(groundPlaneSize, groundPlaneHeight, groundR, groundG, groundB, groundPlaneAmbient, groundPlaneDiffuse, groundPlaneSpecular, groundPlaneShininess);
    glEndList();
}

void changeColor_callback(Fl_Button* button, void*) 
{
    if (groundPlane == ON) {
        clickedCnt += 1;
        if (clickedCnt != colorChangedCnt) {
            if (colorChangedCnt < 3) {
                groundR = groundColors[colorChangedCnt][0];
                groundG = groundColors[colorChangedCnt][1];
                groundB = groundColors[colorChangedCnt][2];
                colorChangedCnt++;
                //std::cout << "currColor: " << groundR << ". " << std::endl;
            }
            else{
                colorChangedCnt = 0;
                groundR = 0.81; groundG = 0.81; groundB = 0.55;
            }
            clickedCnt = colorChangedCnt;
        }
    }
    double groundPlaneHeight = 0.0;
    double groundPlaneSize = 200.0;
...
int main(int argc, char **argv) 
{
  ...
  //changeColor_button->value(changeColor);
